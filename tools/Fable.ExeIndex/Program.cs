...
static void RunExportScriptBank(PeImage pe, DumpStore store, GameInstall? install)
{
    ...
    native.AppendLine("| PlayAVI | `00CCA26D` | first arg required else `jmp 00CD17FD`. Prefix `Data\\Video\\` via `0099F570`, `vtbl+1476` **`0088F890`** → `0040D2A0` then blocking **`006286F0(edx=0x1B)`**. `0099C1E0` rewrites wide `.xmv` → `.wmv`. Open `00A3B9D0` CoCreate FilterGraph/`IGraphBuilder` (`0x12AB174`/`0x12A9934`). Custom renderer `00A3B510` (base `00CA7360`), `AddFilter` vtbl+12 name **`Fable Texture Renderer Filter`** (`0x129D1AC`), `RenderFile` vtbl+52 when `.wmv`/`.asf`. Open QI: IMediaControl `0x12AA094` / IMediaPosition `0x12AA064` / IMediaSeeking `0x12A9A04` / IMediaEvent `0x12AA084` / IBasicAudio `0x12AA054` — **no** `IVideoWindow` / `GetCurrentImage`. `IPin` is a separate heap pin (`00CA6A30` alloc `0xE0`, ctor `00CA4F40`); CBasePin QI `00CA7CE0` returns **this+12** for IID `0x12A9B04`. Filter +12 is `IBaseFilter` (`00CA7500`), not `IPin`. IPin QueryAccept `00CA84C0` → filter vtbl+176 **`00A3B590`**: `FORMAT_VideoInfo` + `MEDIATYPE_Video` + RGB24 only. GetMediaType `00CA84F0` is `0x8000FFFF` (no advertised types). Connection `00A3B5F0` reads `VIDEOINFOHEADER` biWidth/abs(biHeight); stride `((w+1)*3)&~3`; D3D format 21/25 else `0x8004022A`. Copy is C++ vtbl+172 **`00A3B730`** (not mid-fn `00A3B740`): `GetPointer` (+12) RGB24 → `009FA450` `LockRect`, no V flip, `SetEvent([player+124])`. Present `WaitForSingleObject` **33** ms then `009DC870` into the **game** backbuffer, leftover×0.5. `00A3B130` `Run` retry 50. Skip DIK 1/57/28/62. `jmp 00CD17F8`. **No** `vtbl+28`. |");
...
static void RunTraceScriptRuntime(PeImage pe, DumpStore store)
{
    ...
    var links = new List<IndexLink>
    {
        ...
        WriteWalkPart(pe, store, family, "PlayAVI player 006286F0", 0x006286F0, 220),
        ...
        WriteFnPart(pe, store, family, "PlayAVI copy sample 00A3B730", 0x00A3B730, 80, stopOnRet: false),
        ...
        WriteWalkPart(pe, store, family, "PlayAVI renderer QI 00CA6080", 0x00CA6080, 40),
        WriteWalkPart(pe, store, family, "PlayAVI pin QI 00A3BD00", 0x00A3BD00, 8),
        WriteVtblPart(pe, store, family, "PlayAVI renderer IBaseFilter 0129D08C", 0x0129D08C, 48),
        WriteVtblPart(pe, store, family, "PlayAVI renderer IPin 0129D04C", 0x0129D04C, 16),
        ...
        WriteGuidPart(pe, store, family, "PlayAVI IMediaControl IID 012AA094", 0x012AA094),
        WriteGuidPart(pe, store, family, "PlayAVI IMediaPosition IID 012AA064", 0x012AA064),
        WriteGuidPart(pe, store, family, "PlayAVI IMediaSeeking IID 012A9A04", 0x012A9A04),
        WriteGuidPart(pe, store, family, "PlayAVI IMediaEvent IID 012AA084", 0x012AA084),
        WriteGuidPart(pe, store, family, "PlayAVI IBasicAudio IID 012AA054", 0x012AA054),
        WriteGuidPart(pe, store, family, "PlayAVI IOverlay IID 012AAB04", 0x012AAB04),
        WriteGuidPart(pe, store, family, "PlayAVI IQualityControl IID 012A99B4", 0x012A99B4),
        WriteGuidPart(pe, store, family, "PlayAVI IBaseFilter IID 012A9AA4", 0x012A9AA4),
        WriteGuidPart(pe, store, family, "PlayAVI IMediaFilter IID 012A9AB4", 0x012A9AB4),
        WriteGuidPart(pe, store, family, "PlayAVI IPersist IID 012C0B74", 0x012C0B74),
        WriteWalkPart(pe, store, family, "PlayAVI CBaseRenderer ctor 00CA7360", 0x00CA7360, 40),
        WriteWalkPart(pe, store, family, "PlayAVI leftover QI 00CA4780", 0x00CA4780, 30),
        WriteWalkPart(pe, store, family, "PlayAVI CBaseFilter QI 00CA7500", 0x00CA7500, 50),
        WriteWalkPart(pe, store, family, "PlayAVI CUnknown QI 00CACCB0", 0x00CACCB0, 30),
        WriteFnPart(pe, store, family, "PlayAVI IMemInputPin Receive 00CA6050", 0x00CA6050, 20, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI Receive helper 00CA5F50", 0x00CA5F50, 80, stopOnRet: false),
        WriteWalkPart(pe, store, family, "PlayAVI IBaseFilter slot10 00CA6580", 0x00CA6580, 20),
        WriteWalkPart(pe, store, family, "PlayAVI IBaseFilter slot11 00CA65B0", 0x00CA65B0, 40),
        WriteWalkPart(pe, store, family, "PlayAVI GetPin 00CA5850", 0x00CA5850, 30),
        WriteFnPart(pe, store, family, "PlayAVI copy neighbour 00A3B590", 0x00A3B590, 20, stopOnRet: false),
        WriteWalkPart(pe, store, family, "PlayAVI CBaseFilter ctor 00CA61D0", 0x00CA61D0, 40),
        WriteWalkPart(pe, store, family, "PlayAVI GetClassID 00A3BDC0", 0x00A3BDC0, 20),
        WriteWalkPart(pe, store, family, "PlayAVI Stop 00CA77A0", 0x00CA77A0, 30),
        WriteWalkPart(pe, store, family, "PlayAVI Pause 00CA79F0", 0x00CA79F0, 30),
        WriteWalkPart(pe, store, family, "PlayAVI IMediaFilter Run 00CA48F0", 0x00CA48F0, 40),
        WriteWalkPart(pe, store, family, "PlayAVI GetState 00CA6A30", 0x00CA6A30, 40),
        WriteWalkPart(pe, store, family, "PlayAVI SetSyncSource 00A3BCD0", 0x00A3BCD0, 16),
        WriteWalkPart(pe, store, family, "PlayAVI GetSyncSource 00CA6450", 0x00CA6450, 20),
        WriteWalkPart(pe, store, family, "PlayAVI QueryFilterInfo 00CA6670", 0x00CA6670, 30),
        WriteWalkPart(pe, store, family, "PlayAVI JoinFilterGraph 00A3BCE0", 0x00A3BCE0, 16),
        WriteWalkPart(pe, store, family, "PlayAVI QueryVendorInfo 00CA55E0", 0x00CA55E0, 16),
        WriteWalkPart(pe, store, family, "PlayAVI GetPinCount 00CA53A0", 0x00CA53A0, 20),
        WriteWalkPart(pe, store, family, "PlayAVI GetPin 00CA53B0", 0x00CA53B0, 30),
        WriteWalkPart(pe, store, family, "PlayAVI SetMediaType 00CA6B80", 0x00CA6B80, 30),
        WriteWalkPart(pe, store, family, "PlayAVI BreakConnect 00CA6C40", 0x00CA6C40, 30),
        WriteWalkPart(pe, store, family, "PlayAVI CompleteConnect 00CA6E10", 0x00CA6E10, 30),
        WriteFnPart(pe, store, family, "PlayAVI pin Connect 00CA7620", 0x00CA7620, 20, stopOnRet: false),
        WriteWalkPart(pe, store, family, "PlayAVI CompleteConnect helper 00CA7930", 0x00CA7930, 40),
        WriteWalkPart(pe, store, family, "PlayAVI ReceiveConnection 00CA66D0", 0x00CA66D0, 80),
        WriteWalkPart(pe, store, family, "PlayAVI Disconnect 00CA67B0", 0x00CA67B0, 30),
        WriteFnPart(pe, store, family, "PlayAVI ConnectedTo 00CA68F0", 0x00CA68F0, 50, stopOnRet: false),
        WriteWalkPart(pe, store, family, "PlayAVI ConnectionMediaType 00CA6620", 0x00CA6620, 20),
        WriteWalkPart(pe, store, family, "PlayAVI QueryPinInfo 00CA7680", 0x00CA7680, 30),
        WriteFnPart(pe, store, family, "PlayAVI QueryDirection 00CA76F0", 0x00CA76F0, 40, stopOnRet: false),
        WriteWalkPart(pe, store, family, "PlayAVI QueryId 00CAC890", 0x00CAC890, 20),
        WriteWalkPart(pe, store, family, "PlayAVI FindPin 00CA4910", 0x00CA4910, 40),
        WriteWalkPart(pe, store, family, "PlayAVI QueryAccept 00CA4910", 0x00CA4910, 40),
        WriteWalkPart(pe, store, family, "PlayAVI EnumMediaTypes 00CA77F0", 0x00CA77F0, 30),
        WriteFnPart(pe, store, family, "PlayAVI CBaseFilter QI full 00CA7500", 0x00CA7500, 80, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI CBaseRenderer QI full 00CA6080", 0x00CA6080, 80, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI CBasePin QI 00CA7CE0", 0x00CA7CE0, 50, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI IPin COM QI 00CA53C0", 0x00CA53C0, 8, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI IPin QueryAccept 00CA84C0", 0x00CA84C0, 20, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI accept type 00A3B590", 0x00A3B590, 40, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI pin CheckMediaType thunk 00CA5200", 0x00CA5200, 8, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI GetMediaType 00CA84F0", 0x00CA84F0, 8, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI EnumMediaTypes 00CAC170", 0x00CAC170, 50, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI IEnumMediaTypes Next 00CAB160", 0x00CAB160, 80, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI pin ctor 00CA4F40", 0x00CA4F40, 30, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI CBasePin ctor 00CAC260", 0x00CAC260, 30, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI GetPin 00CA6A30", 0x00CA6A30, 80, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI EnumPins 00CAC890", 0x00CAC890, 50, stopOnRet: false),
        WriteVtblPart(pe, store, family, "PlayAVI IPin 012BF3C0", 0x012BF3C0, 20),
        WriteVtblPart(pe, store, family, "PlayAVI pin C++ 012BF408", 0x012BF408, 24),
        WriteVtblPart(pe, store, family, "PlayAVI IEnumMediaTypes 012BF6BC", 0x012BF6BC, 8),
        WriteGuidPart(pe, store, family, "PlayAVI IPin IID 012A9B04", 0x012A9B04),
        WriteGuidPart(pe, store, family, "PlayAVI IMemInputPin IID 012A9A54", 0x012A9A54),
        WriteGuidPart(pe, store, family, "PlayAVI RGB24 012AB604", 0x012AB604),
        WriteGuidPart(pe, store, family, "PlayAVI RGB32 012AB5F4", 0x012AB5F4),
        WriteGuidPart(pe, store, family, "PlayAVI FORMAT_VideoInfo 012AABB4", 0x012AABB4),
        WriteGuidPart(pe, store, family, "PlayAVI MEDIATYPE_Video 012AB884", 0x012AB884),
        WriteGuidPart(pe, store, family, "PlayAVI renderer CLSID 0129D150", 0x0129D150),
        WriteFnPart(pe, store, family, "PlayAVI CEnumPins ctor 00CABE00", 0x00CABE00, 40, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI CEnumPins Next 00CAAFC0", 0x00CAAFC0, 50, stopOnRet: false),
        WriteVtblPart(pe, store, family, "PlayAVI CEnumPins 012BF69C", 0x012BF69C, 8),
        WriteFnPart(pe, store, family, "PlayAVI IPin ConnectedTo 00CA8340", 0x00CA8340, 30, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI IPin QueryDirection 00CA8490", 0x00CA8490, 16, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI IPin QueryPinInfo 00CA8420", 0x00CA8420, 40, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI GetPinCount 00CA48F0", 0x00CA48F0, 8, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI GetPinVersion 00CA79F0", 0x00CA79F0, 8, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI IPin Connect 00CAB470", 0x00CAB470, 80, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI pin C++ QI 00CA89A0", 0x00CA89A0, 40, stopOnRet: false),
        WriteGuidPart(pe, store, family, "PlayAVI IMemInputPin C++ IID 012A9A24", 0x012A9A24),
        WriteVtblPart(pe, store, family, "PlayAVI pin IQualityControl 012BF3AC", 0x012BF3AC, 8),
        WriteWalkPart(pe, store, family, "PlayAVI QueryInternalConnections 00CA6110", 0x00CA6110, 16),
        WriteWalkPart(pe, store, family, "PlayAVI EndOfStream 00CA7920", 0x00CA7920, 20),
        WriteWalkPart(pe, store, family, "PlayAVI GetAllocator 00CA5D50", 0x00CA5D50, 20),
        WriteFnPart(pe, store, family, "PlayAVI GetAllocator exact 00CA5D50", 0x00CA5D50, 24, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI WaitEx 00628A9E", 0x00628A9E, 16, stopOnRet: false),
        WriteFnPart(pe, store, family, "PlayAVI SetEvent 00A3B8EB", 0x00A3B8EB, 12, stopOnRet: false),
        WriteWalkPart(pe, store, family, "PlayAVI NotifyAllocator 00CA5DA0", 0x00CA5DA0, 20),
        WriteFnPart(pe, store, family, "PlayAVI NotifyAllocator exact 00CA5DA0", 0x00CA5DA0, 24, stopOnRet: false),
        WriteWalkPart(pe, store, family, "PlayAVI GetAllocatorRequirements 00CA5DF0", 0x00CA5DF0, 16),
        WriteWalkPart(pe, store, family, "PlayAVI ReceiveMultiple 00CA5E70", 0x00CA5E70, 20),
        WriteWalkPart(pe, store, family, "PlayAVI ReceiveConnection helper 00CAAB30", 0x00CAAB30, 40),
        WriteWalkPart(pe, store, family, "PlayAVI pin AddRef 00A3BD10", 0x00A3BD10, 8),
        WriteWalkPart(pe, store, family, "PlayAVI pin Release 00A3BD20", 0x00A3BD20, 8),
        WriteWalkPart(pe, store, family, "PlayAVI IMemInputPin QI 00A3BD60", 0x00A3BD60, 8),
        WriteWalkPart(pe, store, family, "PlayAVI CreateTexture 009FA280", 0x009FA280, 40),
        WriteWalkPart(pe, store, family, "PlayAVI texture helper 009E3790", 0x009E3790, 20),
        WriteWalkPart(pe, store, family, "PlayAVI present texture 009FA4E0", 0x009FA4E0, 30),
        WriteWalkPart(pe, store, family, "PlayAVI Unlock 009F9DE0", 0x009F9DE0, 20),
        WriteWalkPart(pe, store, family, "PlayAVI open helper 009A4EC0", 0x009A4EC0, 20),
        WriteWalkPart(pe, store, family, "PlayAVI open helper 009F5540", 0x009F5540, 20),
        WriteWalkPart(pe, store, family, "PlayAVI path copy 009BD460", 0x009BD460, 20),
        WriteWalkPart(pe, store, family, "PlayAVI ends-with 0099AF00", 0x0099AF00, 20),
        WriteWalkPart(pe, store, family, "PlayAVI c-str 0099AF10", 0x0099AF10, 16),
        WriteVtblPart(pe, store, family, "PlayAVI pin IQualityControl 0129D030", 0x0129D030, 8),
        WriteVtblPart(pe, store, family, "PlayAVI IPin full 0129D04C", 0x0129D04C, 20),
        WriteCallsPart(pe, store, family, "calls PlayAVI CheckMediaType 00A3B5F0", 0x00A3B5F0),
        WriteCallsPart(pe, store, family, "calls PlayAVI copy 00A3B730", 0x00A3B730),
        WriteCallsPart(pe, store, family, "calls PlayAVI ReceiveConnection 00CA66D0", 0x00CA66D0),
        WriteCallsPart(pe, store, family, "calls PlayAVI CreateTexture 009FA280", 0x009FA280),
        ...
        WriteWalkPart(pe, store, family, "frame dt QPC 009E1BC0", 0x009E1BC0, 40),
        ...
        WriteFnPart(pe, store, family, "FadeIn vtbl+1496 0088E4C0", 0x0088E4C0, 8, stopOnRet: false),
        WriteFnPart(pe, store, family, "fade overlay draw 006496BC", 0x006496BC, 20, stopOnRet: false),
        WriteWalkPart(pe, store, family, "fade overlay alpha 004348D0", 0x004348D0, 20),
        WriteWalkPart(pe, store, family, "fade overlay tick 00434870", 0x00434870, 20),
        WriteWalkPart(pe, store, family, "fade overlay record 0041BEB0", 0x0041BEB0, 40),
        WriteU32Part(pe, store, family, "fade overlay size -8 0125A298", 0x0125A298, 1),
        ...
        WriteWalkPart(pe, store, family, "appearance DEFAULT play 005B37F7", 0x005B37F7, 160),
        WriteFnPart(pe, store, family, "appearance DEFAULT request 005B3A24", 0x005B3A24, 50, stopOnRet: false),
        WriteCallsPart(pe, store, family, "calls appearance DEFAULT play 005B37F7", 0x005B37F7),
        WriteFnPart(pe, store, family, "appearance DEFAULT clothing GUI 005B6881", 0x005B6881, 20, stopOnRet: false),
        WriteFnPart(pe, store, family, "appearance DEFAULT PC_UI_FRAME 005B8743", 0x005B8743, 20, stopOnRet: false),
        ...
        WriteWalkPart(pe, store, family, "cutscene actor map 00CDBF70", 0x00CDBF70, 16),
        ...
        WriteFnPart(pe, store, family, "SneakTo wait poll 00CC0F1A", 0x00CC0F1A, 16, stopOnRet: false),
        WriteFnPart(pe, store, family, "SneakTo wait yield 00CC0ECD", 0x00CC0ECD, 16, stopOnRet: false),
        ...
        WriteFnPart(pe, store, family, "LookInDirection token 00CC3F73", 0x00CC3F73, 40, stopOnRet: false),
        WriteFnPart(pe, store, family, "LookInDirection apply 00CC4009", 0x00CC4009, 16, stopOnRet: false),
        WriteWalkPart(pe, store, family, "LookInDirection vtbl+1896 0089BDF0", 0x0089BDF0, 80),
        WriteU32Part(pe, store, family, "LookInDirection 1/360 01238E00", 0x01238E00, 1),
        WriteFnPart(pe, store, family, "cutscene skip predicate 00CBEB7E", 0x00CBEB7E, 12, stopOnRet: false),
        WriteFnPart(pe, store, family, "cutscene vector1 copy 00CC017C", 0x00CC017C, 20, stopOnRet: false),
        WriteWalkPart(pe, store, family, "cutscene skip vtbl+168 00894440", 0x00894440, 20),
        WriteWalkPart(pe, store, family, "cutscene skip vtbl+176 00893B00", 0x00893B00, 20),
        WriteU32Part(pe, store, family, "cutscene skip global 0143E8F4", 0x0143E8F4, 1),
...
static void RunTraceNewGame(PeImage pe, DumpStore store)
{
    ...
    var links = new List<IndexLink>
    {
        ...
        WriteFnPart(pe, store, family, "CHeroCentreDef factory 004D86F0", 0x004D86F0, 20),
        WriteFnPart(pe, store, family, "CQuestCardDef factory 004E2333", 0x004E2333, 16, stopOnRet: false),
        WriteFnPart(pe, store, family, "CQuestCardDef persist ctor 004E00BC", 0x004E00BC, 40),
        WriteFnPart(pe, store, family, "CFlammableDef factory 004E3DC3", 0x004E3DC3, 16, stopOnRet: false),
        WriteFnPart(pe, store, family, "CFlammableDef persist ctor 004E284B", 0x004E284B, 24),
        ...
        WriteFnPart(pe, store, family, "Light apply exact 00F39D40", 0x00F39D40, 24, stopOnRet: false),
        WriteFnPart(pe, store, family, "c35 setter default stack 0098B319", 0x0098B301, 16, stopOnRet: false),
        WriteFnPart(pe, store, family, "PALSKIN packer dest-null 00BD2D90", 0x00BD2D90, 40, stopOnRet: false),
        WriteFnPart(pe, store, family, "Camera +536 helper gate 00B314E0", 0x00B314E0, 16, stopOnRet: false),
        WriteFnPart(pe, store, family, "Water empty out 00B7A865", 0x00B7A865, 4, stopOnRet: false),
        ...
        WriteFnPart(pe, store, family, "Water draw empty je 00B7851D", 0x00B7851D, 8, stopOnRet: false),
        WriteFnPart(pe, store, family, "Water draw second gate 00B72180", 0x00B72180, 20, stopOnRet: false),
        WriteCallsPart(pe, store, family, "calls Water draw second gate 00B72180", 0x00B72180),
        ...
        WriteWalkPart(pe, store, family, "C3D primitive serialize 00A8FD40", 0x00A8FD40, 200),
        WriteFnPart(pe, store, family, "C3D animated block write 00A8E770", 0x00A8E770, 60),
        WriteWalkPart(pe, store, family, "C3D animated block read head 00A8E8A0", 0x00A8E8A0, 200),
        WriteWalkPart(pe, store, family, "C3D animated block read tail 00A8EB10", 0x00A8EB10, 200),
        WriteFnPart(pe, store, family, "serialize u8 00993E30", 0x00993E30, 16, stopOnRet: true),
        WriteFnPart(pe, store, family, "serialize 1-byte 00993E50", 0x00993E50, 8, stopOnRet: true),
        WriteFnPart(pe, store, family, "serialize u16 00993E80", 0x00993E80, 8, stopOnRet: true),
        WriteFnPart(pe, store, family, "serialize u32 00993EA0", 0x00993EA0, 8, stopOnRet: true),
        ...
        WriteFnPart(pe, store, family, "C3DMeshLODInfo ready 00A23DE0", 0x00A23DE0, 8, stopOnRet: false),
        WriteFnPart(pe, store, family, "CreateTexture DXT1 009BE8B0", 0x009BE8B0, 30, stopOnRet: false),
        WriteFnPart(pe, store, family, "CreateTexture DXT1 probe 009BE800", 0x009BE800, 16, stopOnRet: false),
        WriteFnPart(pe, store, family, "CreateTexture DXT1 named 009BE830", 0x009BE830, 20, stopOnRet: false),
        WriteFnPart(pe, store, family, "CreateTexture DXT3 009BE870", 0x009BE870, 24, stopOnRet: false),
        WriteFnPart(pe, store, family, "CreateTexture format table 009BE590", 0x009BE590, 40, stopOnRet: false),
        WriteFnPart(pe, store, family, "Init Graphics 00416C8A", 0x00416C8A, 80, stopOnRet: false),
        WriteCallsPart(pe, store, family, "calls CreateTexture DXT1 009BE8B0", 0x009BE8B0),
        WriteVtblPart(pe, store, family, "C3DMeshLODInfo vtbl 0129CDB4", 0x0129CDB4, 8),
        WriteFnPart(pe, store, family, "Resource ready 009D54E0", 0x009D54E0, 30, stopOnRet: false),
        WriteFnPart(pe, store, family, "Resource lock helper 00A62920", 0x00A62920, 20, stopOnRet: false),
        WriteFnPart(pe, store, family, "Landscape tile expand ready 00BF3BEC", 0x00BF3BEC, 20, stopOnRet: false),
        WriteFnPart(pe, store, family, "Patch submit ready 00BDC6D9", 0x00BDC6D9, 16, stopOnRet: false),
        WriteCallsPart(pe, store, family, "calls Resource ready 009D54E0", 0x009D54E0),
