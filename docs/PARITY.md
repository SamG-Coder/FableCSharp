...
7. **Animation / bones / cloth.** Parser skips the blocks so static positions survive. No skinning. `009896D0` is `SetVSConstantF` (`[dev+376]`) of `count` float4s at `c[inner+20+offset]`. **11** `E8` callers; pushed offsets are **1, 4, 5, 8, 16** — **never 0**. First-frame `c38` is not written by that wrapper. `00DBDE40` looks up `CREATURE_HERO_CHILD` then `WatchBarrels`. `CThingPlayerCreature::Create` `006AC910` has no play-anim. PALSKIN names appear on init bind (`00BD01B8` family). First-seen kid stays bind-pose. `0089FAA8` does not load `0x1436E9C` or call add-light `00B480E0`; message 16 is only lighting vtbl[7] `00B481E0`.
