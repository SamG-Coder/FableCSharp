...
| Fine terrain mesh | 1-unit quads still exist for sampling. The **drawn** landscape is the STB tile mesh: index **strip** for adaptive tiles, 17×17 quads only when `v=289` and the lattice is complete. Treating 70%-filled adaptive tiles as a grid skipped ~1120 Lookout quads (sky holes). Draw bit `0x40` walks patches through **`00BDC2D0`**: 4-plane AABB vs camera `+448` (stride 16). `n[i]&gt;0` picks patch `+168` (min) else `+180` (max); reject when `n·p &gt; d` (fully outside). Extract is `00B2FD60` / store `00A42140`. Missing AABB submits every cell. Plane *values* stay on the camera object. | `LevFormatTests` |
...
| Landscape UV + bind | Tile verts are 15 bytes and have **no UV**. Draw `00B6B0B0` bit `0x40` walks patches `00BDC2D0` (4-plane frustum) then `00BF4570` per 16 m cell (`[patch+268/270] >> 4`). `SetTexture` (`[dev+260]`) stage 0 from cell+1468, stage 1 at `00BF5491`. `PSHADER_LANDSCAPE_FOREGROUND` is `mul_x2_sat r0.xyz, t1, v0` — **RGB is stage 1**. First-seen bind puts primary `TextureId` on t1 and `TextureId1` on t0. Strip material is sampled at the triangle centroid, not vertex A. VS wrapper `00989A60` is `SetVertexShaderConstantF` (`[dev+376]`, count 1). Global `0x1436E14` is static object `0x13BC470`. `[this+4]` is `&this+24` (`0098AD50`). `[this+4]+20` is inner `+20`, zeroed in `0098D4A0` (`xor edi,edi` / `mov [ebx+4], edi` on the `+16` subobject). **Slot is the D3D register.** Per-cell writes: table `0x0139C5D8` (`±0.125` UV), table `0x0139C614` (more `±0.125` rows) to **c3**, and **c1** as `(1,0)` / `(0,-1)` layer flip. Landscape **c3 is UV**, not ambient. Layer types 3/4/5 enqueue on water. After the pass `00B67510` unbinds 0/1/2. | `LevFormatTests`, `ShaderFormatTests`, exe `00BF50E0` / `00989A60` / `0098D4A0` / `0x0139C5D8` |
...
5. **GPU texturing leftovers.** Two albedo samplers, no atlas, no mipmaps, no bump/reflection/illumination maps, no DXT-on-GPU. No shadow-buffer / colbuff variants. Layer-type UV flip (`c1` = `(1,0)` vs `(0,-1)`) and stages 2+ stay unread.
