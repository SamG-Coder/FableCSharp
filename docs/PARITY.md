...
17. **CEngineLightingManager register maps.** `CEngineVSConstantLayoutLights` ctor `00BDB400`: **`[+84]=19`**, `[+76]=2`, **`[+96]=35`**, `[+100]=1`, **`[+108]=31`** count **4**, **`[+20]=38`** count **58**. Flush `0098A540` uploads light *i* at `c[19+2i]`. Apply `00F39D40` writes dirlight **c19/c20**. **c35 first-seen = (0,0,0,1)**. Gather `00B46280` (cap 8, per-cell asks 5) builds a nearby-light list from a 16-cell grid; pack `00B44F20` writes slot `1..n` via `00989830` and disables leftover slots with `00988640`. Record pack `00B49320` copies pos + colour + `1.0`. **c31 flush `0098A6F6`** is `SetVSConstantF(c31, [wrapper+416], count)` only when packed count **> 1**. `VSHADER_*_DIRLIGHT_FOG` (first-seen bind names) do **not** read `c21`/`c31`. `VSHADER_*_2POINTLIGHTS_FOG` reads **c21–c24** and **c31–c32**. Draw-time 2LIGHTS vs 1-light **selector UNREAD** — do not invent a point-light term on the 1-light VS. Indoor `MARKER_LIGHT` TNG is still the first-seen local light source. PALSKIN `c38` values unread on this graph.
...
7. **Animation / bones / cloth.** Parser skips the blocks so static positions survive. No skinning. New Game function map (`fnmap.md`, 961 fns from StartOakVale seeds, not Lookout) reaches **0** `CTCAnimation*` functions and **0** callers of `009896D0`. `00DBDE40` looks up `CREATURE_HERO_CHILD` then `WatchBarrels`. `CThingPlayerCreature::Create` `006AC910` has no play-anim. PALSKIN names appear on init bind (`00BD01B8` family). First-seen kid stays bind-pose; `c38` values are not written on this graph.
