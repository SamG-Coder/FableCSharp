...
15. **LoadWaterData / CEngineWaterRenderer mesh.** `OpenStaticMaps` mode 1 (`00B4282B`) copies `__ENGINE_SEA_STATIC_MAP_BANK_FILE__` onto the water renderer (`00B6D4D0`, store +1448 / +0x5AC) then looks up `__STATIC_MAP_COMMON_HEADER__` (`0x1436ECC`). `LoadWaterData` (`00B41FA0`) looks up `__ENGINE_WATER_STATIC_MAP_BANK_FILE__` (`0x1436EC8`); missing intern jumps to `00B420E4` (no ingest). When a bank exists, `00B6DAF0` reads the first u32 and calls `00B6D6E0`: **`cmp eax, 8` / `jne` → `xor al,al; ret 8`**. Type **8** copies the next two u32s to `[this]`/`[this+4]` and returns 1. StartOakVale has **no** water-prefix STB bank; sea first u32 is **7363**, so `IsLoadableWaterBank` is false. Draw `00B783F0` ORs emptiness of vectors at +508/+512/+520/+524/+532/+536/… and **`je 00B7A865`** when all are empty (`00B7851D`). First-seen landscape no longer emits `442`. Type-8 payload after those two u32s is UNREAD — water mesh still not drawn.
...
7. **Animation / bones / cloth.** Parser skips bone blocks so C3D verts stay bind-pose. Kid **4300** has bones; first-seen VS family is PALSKIN. Ctor `00BD01B8` stores slot 0 `VSHADER_PALSKIN_DIRLIGHT_FOG`, 1–2 `*_2POINTLIGHTS_FOG`, 3–4 `*_4POINTLIGHTS_FOG`, 5 `*_5POINTLIGHTS_FOG`. Bind switch starts at **`00BD3070`** (`push ebp; lea ebp, [esp-112]`; INT3 at `00BD3061` after bone packer `00BD2D90`). First-seen case **`00BD3C36`** uses family+32 remap / +56 (zeros ⇒ slot 0) then **`jmp 00BD3E17`** → compare `[+18068]` vs 1 and **`jmp 00BD4DA6`**. Common tail calls TOD `00B46C80` + pack `00B44F20` — **not** `00989A60(0)`. The c38 writer is only the `[0x1436E84]+148` case at **`00BD42CD`**. Slot **33** (`+148 = 16+33*4`) has **no** New Game registrar: `00B8FAD0` has one `E8` caller (lighting ctor); lighting vtbl+8 writes **15/16**, static `00BA7770` writes **36**, PALSKIN `00BD28A0` writes **37/38**; zero `mov [rm], 33` in `00B20000–00BE0000`. `FirstSeenUploadsPaletteC38` is **false** — do not invent those four floats or an identity palette. Shader manager `00B3CDD4` attaches LayoutLights (`[+20]=38`) as wrapper layout **index 2**. Light flush `0098A540` reads `[wrapper+4]+84` (**19**), so `00989A60(0)` would be **c38** if that case ran. `009896D0` still never pushes offset 0. Bone packer `00BD2D90` writes 64-byte records at `[this+288]` (`shl eax, 6`); first-seen bind path does not upload them. `00DBDE40` has no play-anim. `0089FAA8` does not send message 16.
