...
| Player start + walk | **New game is not Lookout.** Click path: `UI_TEXT_NEW_GAME` `00595B24` → `START_NEW_QUEST` → `Q_NewOakValeIntro` / `StartOakVale` `00DBDE40`. `00CB7940` is a 6-insn **hero-exists** predicate (`[this+44]` then `[hero+5]`), not spawn/pose. `004AA840` is a string destructor (vtbl `0x1238C8C` → `0099A2E0`), not a creature ctor. After `CREATURE_HERO_CHILD` the same fn starts `Q_NewOakValeIntro_PreAttack`, writes **12.0f** (`0x41400000`) at vtbl+2584, looks up **`HerosOldHouse`**, then waits on `[this+80]`. `00DBE3C0` / `00DBEB20` are **post-attack** (`Q__OakValeIntro_PostAttack`, `M_PostAttackStart`) — not first-seen. First-seen is **inside HerosOldHouse** (mesh **6909**, **2** prims: walls **345** + **3180**; roof 3182 / floors 3184 have no prims). Indoor TNG also emits fireplace / door / chairs / cupboards / bookshelves / extra beds / table-lamp (Graphic CRC on every def type, not only OBJECT/CREATURE/BUILDING). Ceiling `MARKER_LIGHT` at (33.91,131.55,22.35): `CTCLight` Active, colour **(130,60,5,255)**, inner **8** / outer **9**. Camera `CAM_OVIF_SHOT2` spline key 0 (FOV **72**). `Diffuse2X` `0098B5E0` is dirty bits, not CULLMODE. Vulkan cull stays none until a SetRenderState dump proves otherwise. D3D Z **LESSEQUAL**. | `WorldSceneTests`, `WorldGeometryTests`, `MeshFormatTests`, `GameBinFormatTests`, exe `newgame-trace` v9 |
...
19. **C3DMeshLODInfo / boolean-alpha / two-sided.** RTTI `C3DMeshLODInfo` (`00A23DE0` is `if ([this+36]) jmp 009D54E0; return 1`). `CStateBlockFunctionAlphaBoolean` / `CEngineStateBlockAlpha` write device slots at +10524/+10544, not a raw `D3DRS_CULLMODE`. C3D **Flag3=1** is the `DegenerateTriangles` material (diffuse 0). **Flag1=1** on kid hair and house mat 3180 is still UNREAD (do not treat as two-sided yet). `scanff 228` found no `SetRenderState` byte-offset form.
