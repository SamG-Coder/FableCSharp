...
| Player start + walk | **New game is not Lookout.** Click path: `UI_TEXT_NEW_GAME` `00595B24` → `START_NEW_QUEST` → `Q_NewOakValeIntro` / `StartOakVale` `00DBDE40`. First-seen is **inside HerosOldHouse**: TNG `CAM_OVIF_SHOT2` spline key 0 (then SHOT3/4/7 at FOV **72**). `NOVStartHSP` (34.45, 129.03, 15.84) is next to bed/table/rug, not an outdoor follow cam. `CView3DThirdPerson` still UNREAD. D3D default Z is **LESSEQUAL**; we match that (was `Less`, which z-fights fill vs strips). Vulkan cull is still none — landscape `Diffuse2X` cull UNREAD. | `WorldSceneTests`, exe `newgame-trace` |
