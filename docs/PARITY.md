...
| Landscape UV + bind | Tile verts are 15 bytes and have **no UV**. Draw `00B6B0B0` bit `0x40` walks patches `00BDC2D0` (4-plane frustum) then `00BF4570` per 16 m cell (`[patch+268/270] >> 4`). `SetTexture` (`[dev+260]`) stage 0 from cell+1468, stage 1 at `00BF5491`. `PSHADER_LANDSCAPE_FOREGROUND` is `mul_x2_sat r0.xyz, t1, v0` — **RGB is stage 1**. First-seen bind puts primary `TextureId` on t1 and `TextureId1` on t0. Strip material is sampled at the triangle centroid, not vertex A. VS table `0x0139C5D8` is **`±0.125`**. Layer types 3/4/5 enqueue on water. After the pass `00B67510` unbinds 0/1/2. | `LevFormatTests`, `ShaderFormatTests`, exe `00BF50E0` / `0x0139C5D8` |
