...
| C3D materials | After the root matrix: `id`, name, `DecalID`, **`DiffuseMapID`**, bump / reflection / illumination, `MapFlags`, `SelfIllumination`, then **4 flag bytes**. House `DegenerateTriangles` (diffuse 0) is **Flag3=1**. Kid hair and house mat 3180 are **Flag1=1**. First-seen landscape / static-lit apply **CCW** with no Flag1 test; NONE passes apply `0x01396FB8` unconditionally and restore CCW after the draw. **Flag1 is not the first-seen NONE selector** (`FirstSeenAppliesCullNoneFromFlag1=false`). Two-sided meaning of Flag1 is still UNREAD. | `MeshFormatTests`, `ScenePassTests` |
...
19. **C3DMeshLODInfo / boolean-alpha / two-sided.** RTTI `C3DMeshLODInfo` (`00A23DE0` is `if ([this+36]) jmp 009D54E0; return 1`). `CStateBlockFunctionAlphaBoolean` / `CEngineStateBlockAlpha` write device slots at +10524/+10544. First-seen landscape `00B24850` / static-lit `00BB2540` **do** set `D3DRS_CULLMODE` to **3** (`D3DCULL_CCW`) via `0x01396FB0` with **no Flag1 test**. `0x01396FB8` = NONE is applied **unconditionally** at the start of other primitive passes (`00B89C30` at `00B8B0F7`, `00BBE090` at `00BBE717`, `00BC3F30` at `00BC4060`) and **restored to CCW** after the draw (`00BBE8BB` / `00BC4AEF`). Layer switch `00BBC130` (`[arg+8]-7` jump table) routes one case to `00BBE090`. C3D **Flag3=1** is `DegenerateTriangles`. **Flag1=1** on kid hair / house 3180 is still UNREAD as two-sided; it is **not** the first-seen NONE selector (`FirstSeenAppliesCullNoneFromFlag1=false`). Layer type **20** selects NONE-draw `00BBE090` (only `E8` is `00BBC210`). First-seen PALSKIN `00BD71B0` (kid hair 793) tests `[this+8]` as an enable byte and never calls `00BBC130`. First-seen static-lit `00BB2540` (house 3180) never calls it either. Zero `mov [reg+8], 20` in `00A00000–00C00000`. **`FirstSeenFlag1WritesLayerType20=false`**. Kid hair `MapFlags=1` like face/torso; only hair has Flag1=1. First-seen PALSKIN bind (pass 4 through `00BD35ED`) writes **D3DRS_SRCBLEND=5** (`0x01396F78` SRCALPHA) and **DESTBLEND=6** (`0x01396F7C` INVSRCALPHA); slot init `00A047EA`/`00A04825` is RS 19/20. Alphablend enable `+10424` is 1. **No Flag1 test** (`FirstSeenPalskinSrcAlphaBlend=true`, `FirstSeenFlag1SelectsAlphaBlend=false`). Skinned C3D (kid) draws with that blend; house 3180 has no bones and stays opaque. Two-sided meaning still UNREAD.
