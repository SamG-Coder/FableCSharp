...
| Invented N·L `0.28 + 0.72 * n·sun` with guessed sun `(0.35,0.25,0.90)` | LayoutLights `+84=19`, apply `00F39D40` + ctor: **c19=(0,1,0,0)**, **c20=(0.25)×3+1**. Not a 0.28/0.72 scale. | `ShaderFormatTests.First_seen_vs_read_c20_and_c35`, exe `00BDB400` / `00F39D40` / `00B483A0` |
...
17. **CEngineLightingManager register maps.** `CEngineVSConstantLayoutLights` ctor `00BDB400`: **`[+84]=19`**, `[+76]=2`, **`[+96]=35`**, `[+108]=31`. Flush `0098A540` uploads light *i* at `c[19+2i]`. Apply `00F39D40` writes count=2: **c19 = dir xyz + w0**, **c20 = colour**. Lighting ctor `00B482A0` + TOD index 0: dir **`(0,1,0,0)`** (`[esi+48]`), colour **`(0.25,0.25,0.25,1)`** (record0 `0x3E800000`). First-seen VS `DP3` `c19`, `MAD`/`MUL` `c20`, `LIT` `c35`. **c35 float4 still UNREAD** (LIT). Indoor `MARKER_LIGHT` is point-light slot 1+ (`c21`…), not c20.
