...
| Player start + walk | **New game is not Lookout.** Click path: `UI_TEXT_NEW_GAME` `00595B24` → `START_NEW_QUEST` `004B5080` → `AddTestQuest` → `Q_NewOakValeIntro` / `StartOakVale` `00DBDE40` / `CREATURE_HERO_CHILD`. WLD `NewRegion 4` Contains West/East/Garden and Sees fillers+seas. `NOVStartHSP` at (34.45, 129.03) next to `HerosOldHouse`. First-seen instances **4300** `MESH_YOUNGHERO_02` at that HSP. Fly camera sits at HSP + half the kid mesh height (mesh Z span × 0.01), looking along TNG forward × 8. `CView3DThirdPerson` offset still UNREAD. Childhood TNG has **no** `REGION_EXIT_POINT`. | `WorldSceneTests`, `WorldGeometryTests.New_game_oakvale_loads_contains_and_sees_maps`, exe `newgame-trace` |
...
| Uncovered 1 m cells | Oakvale adaptive strips omit many `GROUND_PATH_*` 1 m cells. Cover test is **cell-centre in triangle**, not strip AABB (AABB false-covers holes inside large tris). Leftover land/path/water cells come from the stamped fine heightfield. `WATER_*` / `SEA_*` bind `LANDSCAPE_WATER` **442**. Blank fillers (`StartOakVale_Filler_05`, `PicnicArea_Filler_03`) have STB tiles but every WAD slot is `0xFF` — emit the tile mesh with `LANDSCAPE_GRASS_PLAIN` **414**. `INVALID_*` still skipped. Real water mesh still unread. | `LevFormatTests` |
