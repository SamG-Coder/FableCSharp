...
| Player start + walk | WLD map 1 / `Maps[0]` is LookoutPoint. Spawn is `HOLY_SITE_PLAYER_START` `MAIN_START_POSITION` (102.9, 74.1), facing `RHSetForward`. `CTCDRegionExit` persist (`0077947D`) stores `Active`, `Radius` (Lookout 3.5 / 4.0), `MessageRadius` (UI, 11–12), `EntranceConnectedToUID`. Walk XY into `Radius` of an `Active` exit, then place the hero at the dest `REGION_ENTRANCE_POINT`. The arch `OBJECT_REGION_TRANSITION_GATE` is the mesh, not the trigger. `Active=FALSE` exits (GuildExterior FireHeart) stay shut. | `WorldSceneTests`, exe `CTCDRegionExit` |
