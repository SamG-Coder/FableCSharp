...
| Player start + walk | **New game is not Lookout.** WLD `NewRegion 4` `StartOakVale` (`TXT_REGION_OAKVALE`) contains `StartOakValeWest` / East / MemorialGarden. QST `AddTestQuest("Q_NewOakValeIntro","NOVStartHSP")`. TNG+GTG `NOVStartHSP` is on WLD map 203 `StartOakValeWest` at (34.45, 129.03), next to `HerosOldHouse`. Exe `00DBDE4A` `StartOakVale`, `00DBDF09` `CREATURE_HERO_CHILD`. First-seen instances **4300** `MESH_YOUNGHERO_02` at that HSP (the kid is not a TNG prop). Childhood TNG has **no** `REGION_EXIT_POINT` (gate `NOVI_BlockingGate`). WLD `Maps[0]` Lookout + `MAIN_START_POSITION` (102.9, 74.1) is the adult overworld first map. `CTCDRegionExit` persist (`0077947D`) stores `Active`, `Radius`, `MessageRadius`, `EntranceConnectedToUID`. Walk XY into `Radius` of an `Active` exit, then place the hero at the dest `REGION_ENTRANCE_POINT`. | `WorldSceneTests`, `WorldGeometryTests.New_game_oakvale_loads_contains_and_sees_maps`, exe `StartOakVale` / `CTCDRegionExit` |
...
| Landscape UV + bind | Tile verts are 15 bytes and have **no UV**. Draw `00B6B0B0` bit `0x40` walks patches `00BDC2D0` (4-plane frustum) then `00BF4570` per 16 m cell (`[patch+268/270] >> 4`). `SetTexture` (`[dev+260]`) stage 0 from cell+1468, stage 1 at `00BF5491`. VS constants via `00989A60` → D3D `SetVertexShaderConstantF` count 1. Table `0x0139C5D8` is **`±0.125`** (`1/8`), not `1/16`. `VSHADER_LANDSCAPE_FOREGROUND` `mad oT0` from world XY. One vs two layers also writes VS `c1` `(1,0,0)` / `(0,-1,0)`. Layer types 3/4/5 enqueue on `CEngineWaterRenderer+0x268` (`00BE7BE0`). After the pass `00B67510` unbinds stages 0/1/2. | `LevFormatTests`, exe `00BF4570` / `0x0139C5D8` |
