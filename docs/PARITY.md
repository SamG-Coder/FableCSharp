...
| Player start + walk | **New game is not Lookout.** Click path: `UI_TEXT_NEW_GAME` `00595B24` → `START_NEW_QUEST` → `Q_NewOakValeIntro` / `StartOakVale` `00DBDE40`. First-seen is **inside HerosOldHouse** (`BUILDING_OAKVALE_HOUSE_MEDIUM_SINGLE_FLOOR_BUYABLE` → graphics.big **6909**, **2** primitives: ext walls **345** + **3180**; materials `floors etc` 3184 and `roof` 3182 have **no** primitives). Indoor TNG: bed **6556**, table **7583**, lamp **7544**, rug **4901**. Camera `CAM_OVIF_SHOT2` spline key 0 (SHOT3/4 FOV **72**). `Diffuse2X` `0098B5E0` is a state-block dirty-bit applicator (`or [this+984], …`); first `ret` is arg==-1. **CULLMODE not in that function.** `00A0AA20` is `ret 4`. Vulkan cull stays none until a SetRenderState dump proves otherwise. D3D Z **LESSEQUAL**. | `WorldSceneTests`, `WorldGeometryTests`, `MeshFormatTests`, `GameBinFormatTests`, exe `newgame-trace` v7 |
