...
17. **CEngineLightingManager register maps.** `CEngineVSConstantLayoutLights` ctor `00BDB400`: **`[+84]=19`**, `[+76]=2`, **`[+96]=35`**, `[+100]=1`, `[+108]=31` count 4, **`[+20]=38`** count **58** (PALSKIN bone window). Flush `0098A540` uploads light *i* at `c[19+2i]`. Apply `00F39D40` writes count=2: **c19 = dir xyz + w0**, **c20 = colour**. Lighting ctor `00B482A0` zeros TOD bytes `[+224/+225/+226]` and writes record0 **`+32 = (0,0,0,1)`**. TOD update `00B46C80` (dirty `[+228]` bit 0, set to `0xF` in ctor) copies that float4 to `[esi+104]` and calls setter `0098B2C0` index 0 — the only caller. Flush `0098A760` is `SetVertexShaderConstantF(c35, [wrapper+404], 1)` then clears dirty `0x400`. First-seen VS `DP3` `c19`, `MUL` `c20`, **`MAD` `c35`**. **c35 first-seen = (0,0,0,1)**. Indoor `MARKER_LIGHT` is point-light slot 1+ (`c21`…), not c20. PALSKIN `c38`… values and `c31`×4 still UNREAD.
...
7. **Animation / bones / cloth.** Parser skips the blocks so static positions survive. No skinning. `00DBDE40` looks up `CREATURE_HERO_CHILD` (`[+280]`) then `call [vtbl+376]` and starts `WatchBarrels` — no animation name. `CThingPlayerCreature::Create` `006AC910` allocates, `ConstructFromParams` `006A9DD0` (attaches `CTCPhysicsControlled`), packs default axes **(0,0,1)/(0,1,0)** via `006A06E0`, then `004C9CA0`. No `CTCAnimationComplex` / play-anim call. `CPlayer::CreateCharacter` `00489D40` copies position from `[thing+96]+12` (`006A4D00`) and axes from `vtbl+288`; a +1 XY branch is only when `[player+40]` differs from `00449700`. PALSKIN upload `009896D0` is `SetVSConstantF(c[38+offset], data, count)`. First-frame bone matrices still UNREAD; kid stays bind-pose.
