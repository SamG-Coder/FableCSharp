...
| Uncovered 1 m cells | Oakvale adaptive strips omit many `GROUND_PATH_*` 1 m cells (village 514) and `WATER_GREYCLIFF_ET` (219). 17×17 implicit tiles all emit. Fill leftover land/path/water cells from the stamped fine heightfield. `WATER_*` / `SEA_*` bind `LANDSCAPE_WATER` **442** (not token-scored cliff). Real water mesh still unread. | `LevFormatTests` |
