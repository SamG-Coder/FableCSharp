...
| Tile extras | After XYZ: **11-11-10** packed normal + 3 bytes. Lookout: 289/289 unit Z-up; byte0 of the triple is always `0xFF`. `VSHADER_LANDSCAPE_FOREGROUND` tokens read `c20`/`c35` (and `c3`, which per-cell `00989A60` also fills with UV `±0.125`). Tile RGB is not oD0. Multiplying albedo by (1, ~0.5, ~0.5) made Lookout magenta. Drawn verts use the stored vertex colour (white) until `c20`/`c35` values are dumped. | `LevFormatTests`, `WorldGeometryTests`, `ShaderFormatTests` |
...
| Invented N·L `0.28 + 0.72 * n·sun` with guessed sun `(0.35,0.25,0.90)` | First-seen VS read `c20`/`c35`; those float4s are unread. CPU N·L is not that path. | `ShaderFormatTests.First_seen_vs_read_c20_and_c35`, exe `0098A6C0` |
...
17. **CEngineLightingManager register maps.** First-seen VS tokens **do** read **`c20` and `c35`** (`VSHADER_STATIC_DIRLIGHT_FOG`, `VSHADER_LANDSCAPE_FOREGROUND`, `VSHADER_PALSKIN_DIRLIGHT_FOG`) plus `c2`/`c18` (fog) and `c5`–`c8` (transform). Scene pass calls `00B46C80`/`00B46890` on `0x1436E9C`; TOD blend copies 112-byte records into `[this+72]`/`+104` then `00989830(0)` / `0098B2C0(0)`. Light flush `0098A6C0` uploads from `[inner+84]` (start register) — **the integer at +84 is UNREAD**. Indoor `MARKER_LIGHT` is `0089FAA8` → `00640BB0`. Invented CPU N·L (`0.28+0.72*dot`) was removed; PS still `mul`/`mul_x2` of `v0` (vertex colour, currently white). **c20/c35 float4 values UNREAD.**
