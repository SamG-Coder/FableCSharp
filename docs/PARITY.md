...
| C3D materials | After the root matrix: `id`, name, `DecalID`, **`DiffuseMapID`**, bump / reflection / illumination, `MapFlags`, `SelfIllumination`, then **4 flag bytes**. House `DegenerateTriangles` (diffuse 0) is **Flag3=1**. Kid hair and house mat 3180 are **Flag1=1** (two-sided / alpha still UNREAD). | `MeshFormatTests` |
...
19. **C3DMeshLODInfo / boolean-alpha / two-sided.** RTTI `C3DMeshLODInfo` (`00A23DE0` is `if ([this+36]) jmp 009D54E0; return 1`). `CStateBlockFunctionAlphaBoolean` / `CEngineStateBlockAlpha` write device slots at +10524/+10544. First-seen landscape / static-lit **do** set `D3DRS_CULLMODE` to **3** (`D3DCULL_CCW`) via `0x01396FB0`. `calldisp 0xE4` finds `SetRenderState` (`[dev+228]`); flush is `00A044E0`. `0x01396FB8` = NONE is used by other primitive passes (`00B8B0F7`), not the first-seen CCW path. C3D **Flag3=1** is `DegenerateTriangles`. **Flag1=1** on kid hair / house 3180 is still UNREAD as two-sided.
