...
19. **C3DMeshLODInfo / boolean-alpha / two-sided.** RTTI `C3DMeshLODInfo` (`00A23DE0` is `if ([this+36]) jmp 009D54E0; return 1`). `CStateBlockFunctionAlphaBoolean` / `CEngineStateBlockAlpha` write device slots at +10524/+10544. First-seen landscape `00B24850` / static-lit `00BB2540` **do** set `D3DRS_CULLMODE` to **3** (`D3DCULL_CCW`) via `0x01396FB0` with **no Flag1 test**. `0x01396FB8` = NONE is applied **unconditionally** at the start of other primitive passes (`00B89C30` at `00B8B0F7`, `00BBE090` at `00BBE717`, `00BC3F30` at `00BC4060`) and **restored to CCW** after the draw (`00BBE8BB` / `00BC4AEF`). Layer switch `00BBC130` (`[arg+8]-7` jump table) routes one case to `00BBE090`. C3D **Flag3=1** is `DegenerateTriangles`. **Flag1=1** on kid hair / house 3180 is still UNREAD as two-sided; it is **not** the first-seen NONE selector (`FirstSeenAppliesCullNoneFromFlag1=false`).
