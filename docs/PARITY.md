...
15. **LoadWaterData / CEngineWaterRenderer mesh.** `OpenStaticMaps` mode 1 (`00B4282B`) copies `__ENGINE_SEA_STATIC_MAP_BANK_FILE__` onto the water renderer (`00B6D4D0`, store +1448 / +0x5AC) then looks up `__STATIC_MAP_COMMON_HEADER__` (`0x1436ECC`). `LoadWaterData` (`00B41FA0`) looks up `__ENGINE_WATER_STATIC_MAP_BANK_FILE__` (`0x1436EC8`); missing intern jumps to `00B420E4` (no ingest). When a bank exists, `00B6DAF0` reads the first u32 and calls `00B6D6E0`: **`cmp eax, 8` / `jne` → `xor al,al; ret 8`**. Type **8** copies the next two u32s to `[this]`/`[this+4]` and returns 1. StartOakVale has **no** water-prefix STB bank; sea first u32 is **7363**, so `IsLoadableWaterBank` is false. Draw `00B783F0` ORs emptiness of vectors at +508/+512/+520/+524/+532/+536/… and **`je 00B7A865`** when all are empty (`00B7851D`). First-seen landscape no longer emits `442`. Type-8 ingest in `00B6D6E0` consumes **exactly two u32s** (stream `+12` / `+20` advance by 8); their meaning is UNREAD. No type-8 bank on first-seen — water mesh still not drawn. Flag1 still has no proven link to `0x01396FB8` NONE (`00B8B0F7` applies that pass unconditionally).
