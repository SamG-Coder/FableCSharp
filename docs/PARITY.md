...
15. **LoadWaterData / CEngineWaterRenderer mesh.** `OpenStaticMaps` mode 1 (`00B4282B`) copies `__ENGINE_SEA_STATIC_MAP_BANK_FILE__` onto the water renderer (`00B6D4D0`, store +1448 / +0x5AC) then looks up `__STATIC_MAP_COMMON_HEADER__` (`0x1436ECC`). `LoadWaterData` (`00B41FA0`) looks up `__ENGINE_WATER_STATIC_MAP_BANK_FILE__` (`0x1436EC8`) in the opened map and feeds bytes to `00B6DAF0` / `00B6D6E0` (rejects unless first u32 is **8**). Draw `00B783F0` walks vectors at renderer +508/+512/+520/+524 and **returns at `00B7A865`** when they are empty. StartOakVale has **no** type-8 water bank (sea is 7363). First-seen landscape no longer emits `442`. **Payload layout UNREAD** — water mesh still not drawn.
...
7. **Animation / bones / cloth.** Parser skips the blocks so static positions survive. No skinning. New Game function map v18 (`fnmap.md`, 2935 real prologues in StartOakVale ranges, not Lookout) reaches **0** `CTCAnimation*` functions in-range and **7** callers of `009896D0` (`00BAAEA2`, `00BAAED8`, `00BAB04C`, `00BAB2FE`, `00BBFFA6`, `00BC0F81`, `00BC476B`). `00DBDE40` looks up `CREATURE_HERO_CHILD` then `WatchBarrels`. `CThingPlayerCreature::Create` `006AC910` has no play-anim. PALSKIN names appear on init bind (`00BD01B8` family). First-seen kid stays bind-pose; first-frame `c38` values are UNREAD.
