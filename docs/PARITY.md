...
17. **CEngineLightingManager register maps.** Scene pass calls `00B46C80`/`00B46890` on `0x1436E9C` then caches a matrix at device-wrapper +496 (`009881F0`). `00B46C80` blends two **112-byte** records (time-of-day bytes at +224/+225/+226) into `[this+72]`. First-seen indoor `MARKER_LIGHT` (TNG `CTCLight` colour 130,60,5 / r=8..9) is applied by `0089FAA8` → `00640BB0` (BGRA store). **Which VS constant (`c20`/`c35`/`c3`/`c2`/`c18`) either path writes is still UNREAD.** Do not invent a point-light shader.
