...
7. **Animation / bones / cloth.** Parser skips bone blocks so C3D verts stay bind-pose. Kid **4300** has bones; first-seen VS family is PALSKIN. Ctor `00BD01B8` stores slot 0 `VSHADER_PALSKIN_DIRLIGHT_FOG`, 1–2 `*_2POINTLIGHTS_FOG`, 3–4 `*_4POINTLIGHTS_FOG`, 5 `*_5POINTLIGHTS_FOG`. Draw `00BD3C36` uses the same family+32 remap / +56 slots (zeros ⇒ slot 0). Shader manager `00B3CDD4` attaches LayoutLights (`[+20]=38`) as wrapper layout **index 2**. Light flush `0098A540` reads `[wrapper+4]+84` (**19**), so the current layout is LayoutLights at draw; `00989A60(0)` at `00BD4591` is therefore **c38** (one float4 from a runtime object at `[0x1436E84]+148+12`). Those four floats are UNREAD. `009896D0` still never pushes offset 0. `00DBDE40` has no play-anim. First-seen kid stays bind-pose verts; do not invent an identity palette. `0089FAA8` does not send message 16.
...
first-scene code ranges** into `newgame-trace/fnmap.md` (Lookout is not scanned).
Callees outside the ranges are recorded on the caller only. Parse more functions
in those ranges before chasing a single VA.
