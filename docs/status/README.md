...
Point dest still skips DIPUP (`00BADB36`). Type4 apply
is still host dest AABB hover, not recovered
current-inner `0055CB10`. leave #14 open.
`FrontendPresentBodyIsLive` means
Clear/Begin/recovered DIPUP/Present when Device is set —
it does not dest-lock #14. Startup AVI client Present is
blit-only `006286F0` MATCH (`Draw(default)` /
playAviOnly). Remaining leftover #20 is Game-stage:
`PumpGame` still walks `00435530` because `006286F0`
...
exclusive-walk. leave #14 and #20 open (also leave #36
#42 #48 #50 open). Master is still proving **boot /
world clock**, not animation.
...
dest still invented 512,384,512,384; DIP `(0,0)`
stand-in; `DisplayFlushShouldDip(0, 0)` always false;
`009DA9F0` flush is still Note-only
(`DisplayFlushQueueIsNoteOnly`). Point dest still skips
DIPUP (`00BADB36`). Type4 apply is still host dest AABB
hover, not recovered current-inner `0055CB10`. leave #14
open. `FrontendPresentBodyIsLive` means
Clear/Begin/recovered DIPUP/Present when Device is set —
it does not dest-lock #14. Startup AVI client Present is
blit-only `006286F0` MATCH (`Draw(default)` /
playAviOnly). Remaining leftover #20 is Game-stage:
...
(also leave #36 #42 #48 #50 open). CSS unchanged.
...
| Fix frontend colors and text underlay. TextureFile / FrontendTextDraw / LineShaders / Dx9VulkanFrontend. Tests: `A8R8G8B8_surface_bytes_are_decoded_from_bgra_to_rgba`, `Type6_text_has_native_black_underlay_and_colour_pass`, `Frontend_ps_keeps_clock_sprites_white_and_applies_glyph_diffuse`, `Frontend_PRESS_START_is_type_10_with_text_child`. dest still invented. leave #14 and #20 open | PROVEN | `e7eac64` / `A8R8G8B8_surface_bytes_are_decoded_from_bgra_to_rgba` / `Type6_text_has_native_black_underlay_and_colour_pass` / `Frontend_ps_keeps_clock_sprites_white_and_applies_glyph_diffuse` / `Frontend_PRESS_START_is_type_10_with_text_child` |
| Fix frontend compositing and upload stalls. VulkanLineRenderer.Frontend/Textures + FrontendWidgetFactory. Tests: `Frontend_PRESS_START_is_type_10_with_text_child` (EffectiveColour forest tiles), `Frontend_texture_sets_reuse_cached_pixel_storage`. dest still invented. leave #14 and #20 open | PROVEN | `32588e0` / `Frontend_PRESS_START_is_type_10_with_text_child` / `Frontend_texture_sets_reuse_cached_pixel_storage` |
| Match native frontend animation and text rendering. Type-18 swap dwell (`Frontend_type18_primes_then_cycles_zero_dwell_forest_states`); type-6 black underlay then colour pass with Type6OriginPad==2 (`Type6_text_has_native_black_underlay_and_colour_pass`), mouse last in `Frontend_PRESS_START_is_type_10_with_text_child`; vsync FIFO when window.VSync; live client trace (`Forward_lifecycle_trace_is_bounded_and_can_be_disabled`). Also `Press_Start_type18_lists_match_00547500` and `Native_semantic_frontend_present_builds_device_batch`. VA comments on `00547380` / `0052CF40` / `0054EF00` are host helpers — do not file leftover. dest still invented. leave #14 and #20 open | PROVEN | `7b2d2ab` / `Press_Start_type18_lists_match_00547500` / `Frontend_type18_primes_then_cycles_zero_dwell_forest_states` / `Frontend_PRESS_START_is_type_10_with_text_child` / `Type6_text_has_native_black_underlay_and_colour_pass` / `Forward_lifecycle_trace_is_bounded_and_can_be_disabled` / `Native_semantic_frontend_present_builds_device_batch` |
| Recover complete frontend UI schema and parity behavior. New `FrontendUiFieldCatalog.cs` + `FrontendUiSchema.cs`. Schema names/order recovered — dest/DIP/Present are not. Tests: `Every_frontend_UI_entry_matches_the_complete_native_CUIDef_schema`, `Complete_CUIDef_catalog_uses_original_names_and_matching_file_CRCs` (names+CRCs; only offsets 224/228/392/545 locked), `Height_crc_is_fable_crc_of_Height`. `NativeDestTupleUnread` stays true at HEAD. dest still invented. leave #14 and #20 open | PROVEN | `0374d7b` / `Every_frontend_UI_entry_matches_the_complete_native_CUIDef_schema` / `Complete_CUIDef_catalog_uses_original_names_and_matching_file_CRCs` / `Height_crc_is_fable_crc_of_Height` |
| Map frontend states to recovered UI fields. Test `Persist_00631C60_plus189_plus190_are_u8_and_font_is_names_offset` locks HoveredState/Left/Right CRCs; UI_ACCEPT_NEW_PROFILE persist states == 3. dest still invented. leave #14 and #20 open | PROVEN | `b9d37c8` / `Persist_00631C60_plus189_plus190_are_u8_and_font_is_names_offset` |
| Map frontend UI controls from native definitions. Tests: `New_profile_sliders_construct_persisted_arrow_controls_and_ranges`, `New_Profile_per_control_LMB_uses_dest_not_empty_space`, `New_Profile_apply_cancel_hit_rects_are_disjoint`, `Silk_backspace_is_queued_as_the_native_type15_character`. TryChromeHit removal is leftover #48 cleanup — `PlaceTableCellCount3IsNative` still false. `Leftover14OpenForDestPresentNotes` still true (`Native_ids_slots_and_attach_names_match_exe`). dest still invented. leave #14 and #20 open | PROVEN | `168317a` / `New_profile_sliders_construct_persisted_arrow_controls_and_ranges` / `New_Profile_per_control_LMB_uses_dest_not_empty_space` / `New_Profile_apply_cancel_hit_rects_are_disjoint` / `Silk_backspace_is_queued_as_the_native_type15_character` |
...
(`0ace433`). Startup AVI client Present is blit-only `006286F0`
MATCH (`Draw(default)` / playAviOnly). Remaining leftover #20 is
...
(`98c4acc`). Unload-before-next-slot is recovered. Startup AVI
client Present is blit-only `006286F0` MATCH (`Draw(default)` /
playAviOnly). Remaining leftover #20 is Game-stage: `PumpGame`
...
1. **No-save first real region and first *rendered* scene are
   ...
   *intro view* contract (`StartOakValeWest` / `HerosOldHouse` /
   ...
   DrawsChildList walk (`7adf621` / `b4a2c89`).
   `FrontendPresentBodyIsLive` means
   Clear/Begin/recovered DIPUP/Present when Device is
   set — it does not dest-lock #14. WM_CHAR is type 15;
   Press Start / New Profile / New Game are LMB type
   4/6. Type-10 packet is widget +352, not MessageId.
   Type4 apply is still host dest AABB hover, not
   recovered current-inner `0055CB10`. leave #14 open.
   Host current `_frontendWidgets` is still the switched
   screen (input leftover) (`84a8350`). 3D stays
   Compatibility `host.Draw` after Leave. F2 fly is
   Game-only. Press Start `0xE5` is slot `0x14`
   `0059B5D7` (`59fde69`). `[ui+84]` keeps `0x14`/`0x17`
   ...
   Interface is DISPROVEN (moved). dest still invented
   512,384,512,384; DIP `(0,0)` stand-in;
   `DisplayFlushShouldDip(0, 0)` always false;
   `009DA9F0` flush is still Note-only
   (`DisplayFlushQueueIsNoteOnly`). Point dest still
   skips DIPUP (`00BADB36`). Exclusive-walk leftover is
   STALE (tracker already done). Skip VAs `0052F180` /
   `0052F1D0` stay named constants
   (`SkipVtblsAreMethodCalls == false`); that is not
   ...
   LookoutPoint vs first-scene Oakvale intro view
   (leave #4 open). dest invented 512,384,512,384.
   leave #36 open. #5 grok-goal dump still written. Do
   not re-file. Startup AVI client Present is blit-only
   `006286F0` MATCH (`Draw(default)` / playAviOnly).
   Remaining leftover #20 is Game-stage: `PumpGame`
   still walks `00435530` because `006286F0` does not
   own that pump. Do not treat `FABLE_SKIP_STARTUP_AVI`
   as a 3D Draw fix. leave #20 open. leave #14 and #20
   open (also leave #36 #42 #48 #50 open). New Game
   submit now walks `0x4`/`0x40`/`0x20`/`0x100`/
   `0x2000` (`676bf63`); cell DIP is `0x40` only
   (`40037b1`).
