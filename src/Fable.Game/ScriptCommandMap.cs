...
public static class ScriptCommandMap
{
    ...
    [
        ...
        Spec("CameraEffect", 0x00CD1258, 0x00CD12C2, "arg0,arg1,arg2",
            ScriptReturn.CompleteNow, CommandParity.ScriptLayer,
            "atof three; vtbl+1676(a,b,c); jmp 00CD17FD; filter unread"),
        ...
        Spec("DummyEffect", 0x00CCBD62, 0x00CCBE5F, "type,marker,param[,name][,IsTrue]",
            ScriptReturn.CompleteNow, CommandParity.ScriptLayer,
            "00CBF9DE marker; vtbl+404 not 400; jmp 00CC864B"),
        Spec("CreateLight", 0x00CCB933, 0x00CCBB61, "marker,R,G,B,f,f,flag,name,IsTrue",
            ScriptReturn.CompleteNow, CommandParity.ScriptLayer,
            "9 required; 00BFEA70 RGB; vtbl+408; extras if IsTrue; jmp 00CC864B"),
        ...
        Spec("Speak", 0x00CC25FD, 0x00CC27EA, "listener,text[,hold][,mode]",
            ScriptReturn.YieldAfter, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
        ...
        Spec("DialogSpeak", 0x00CC3165, 0x00CC31BC, "listener,text",
            ScriptReturn.YieldAfter, CommandParity.ScriptLayer,
            "vtbl+1456 handle; one vtbl+28; 1472 unread UI"),
        ...
        Spec("WaitActiveDialog", 0x00CC656B, 0x00CC6612, "",
            ScriptReturn.YieldAfterOrWait, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
        ...
        Spec("SneakTo", 0x00CC0CB5, 0x00CC0E5A, "marker[,speed][,wait]",
            ScriptReturn.YieldAfterOrWait, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
            "thing vtbl+20 is 004C72B0 stub; dest+gait via vtbl+16 006A9960; TickMove"),
        Spec("WalkTo", 0x00CC083D, 0x00CC09E2, "marker[,speed][,wait]",
            ScriptReturn.YieldAfterOrWait, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
            "012457FC/0127293C +20=004C72B0; dest+gait 006A5D90 or[this+146],2; no warp"),
        Spec("RunTo", 0x00CC0A79, 0x00CC09E2, "marker[,speed][,wait]",
            ScriptReturn.YieldAfterOrWait, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
            "same dest+gait slot as WalkTo; mode 1"),
        ...
        Spec("Create", 0x00CCC246, 0x00CCC3E6, "type,marker,name[,extra][,suffix][,unique][,IsFalse]",
            ScriptReturn.CompleteNow, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
            "vtbl+364 008A9100 at marker; extras 008ADF90; unique skip; jmp 00CD17F8"),
        ...
        Spec("RemoveExtras", 0x00CC6ACE, 0x00CC6B21, "[IsFalse],limbo|return|marker",
            ScriptReturn.CompleteNow, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
            "hide=!IsFalse; 00BFEBA8 limbo->[ebp+127] vtbl+1812; return->[ebp+19] show vtbl+1892; marker 008AB980; jmp 00CC7076"),
        Spec("return", 0x00CC6B82, 0x00CC6BC4, "RemoveExtras named-arg",
            ScriptReturn.CompleteNow, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
            "00BFEBA8 vs arg1 NOT 00BFEAF8 verb; [ebp+19]=1; skip 008AB980; show+return vtbl+1892 00CC6F74; DISPROVES interpreter stop"),
        Spec("StopMusic", 0, 0, "",
            ScriptReturn.CompleteNow, CommandParity.ScriptLayer,
        ...
        Spec("CameraRotateThing", 0x00CCA5B6, 0x00CCA712, "thing,param,x,y,z",
            ScriptReturn.YieldAfter, CommandParity.ScriptLayer,
            "5 required; vtbl+1616(thing,xyz,param); jmp 00CC907D"),
        ...
        Spec("CameraPath", 0x00CCAF1D, 0x00CCB048, "a,b,c,d,dur",
            ScriptReturn.CompleteNow, CommandParity.ScriptLayer,
            "4 thing lookups; vtbl+1640(pos0,pos2,pos1,pos3,dur); jmp 00CC864B"),
        Spec("SetLightScene", 0x00CD1425, 0x00CD172A, "index",
            ScriptReturn.YieldAfter, CommandParity.ScriptLayer,
            "atoi; +84 defs NAME:r,g,b; +96 comma indices; vtbl+2180; yield if [ebp+103]"),
        Spec("TintScreenOut", 0x00CD11D0, 0x00CD11F7, "seconds",
            ScriptReturn.CompleteNow, CommandParity.ScriptLayer,
            "atof; vtbl+2704([ebp-112],dur); clear hold; jmp 00CD17FD"),
        Spec("TintScreenTo", 0x00CD0CE4, 0x00CD115A, "5floats,rgb,filter",
            ScriptReturn.CompleteNow, CommandParity.ScriptLayer,
            "7 required; RGB*1/255; ALL:/ALLDEF: vtbl+300/320; vtbl+2700 handle"),
        Spec("PutUpYourSwords", 0x00CC9300, 0x00CC9356, "IsTrue?",
            ScriptReturn.CompleteNow, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
            "IsTrue classifies vtbl+788/792 MELEE/RANGED; always vtbl+520; FALSE still sheathes"),
        ...
        Spec("SetBound", 0x00CC11AB, 0x00CC11FD, "IsFalse?",
            ScriptReturn.CompleteNow, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
            "arg0 required; default 1; IsFalse->0; vtbl+1976(actor,flag); jmp 00CC707C; not SetScared 1984"),
        Spec("Killable", 0x00CC1C30, 0x00CC1C82, "IsFalse?",
            ScriptReturn.CompleteNow, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
        ...
        Spec("SetDamageable", 0x00CC1054, 0x00CC10A6, "[ignored]",
            ScriptReturn.CompleteNow, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
            "arg ignored; vtbl+2064(actor,0); extras 008ADF90; jmp 00CC707C; not Killable 2068"),
        Spec("SetAttackable", 0x00CC0FB6, 0x00CC1008, "[ignored]",
            ScriptReturn.CompleteNow, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
        ...
        Spec("TakeObjectFromHero", 0x00CC8846, 0x00CC8898, "item",
            ScriptReturn.CompleteNow, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
            "arg0 required; vtbl+500(name); jmp 00CD17FD; not TakeFromHero 556"),
        ...
        Spec("SetDoorOpen", 0x00CC8A8D, 0x00CC8BEB, "name,IsFalse?",
            ScriptReturn.CompleteNow, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
            "IsFalse(arg1)->vtbl+1704 close else vtbl+1700 open; jmp 00CD17F8"),
        Spec("SetChestOpen", 0x00CC8C14, 0x00CC8D73, "name,IsFalse?",
            ScriptReturn.CompleteNow, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
            "IsFalse(arg1)->vtbl+1744 close else vtbl+1740(thing,0) open; jmp 00CD17F8"),
        ...
        Spec("SetHomePosThing", 0x00CC7CE9, 0x00CC7D3C, "thing",
            ScriptReturn.CompleteNow, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
        ...
        Spec("Release", 0x00CC4610, 0x00CC4663, "",
            ScriptReturn.CompleteNow, new CommandParity(
                CommandStatus.Proven, CommandStatus.Proven, CommandStatus.Proven,
                CommandStatus.Proven, CommandStatus.Partial),
