using System.Collections.Generic;

namespace Fable.Game;

/// <summary>
/// Window / input / Present surface the
/// PE header implements. The engine
/// owns modes, load, AVI, camera, and
/// world submit. The host does not
/// decide New Game, region, or expand.
/// </summary>
public interface IEngineHost
{
    int Width { get; }
    int Height { get; }
    string Title { get; set; }

    /// <summary>
    /// <c>009BEEB0</c> Present. Engine
    /// already chose camera / world / AVI.
    /// </summary>
    void Present(EngineFrame frame);

    void Quit();
}

/// <summary>
/// One frame the engine asks the host
/// to Present. Null world is frontend
/// or loading (clear only).
/// </summary>
public readonly record struct EngineFrame(
    ScriptedCamera Camera,
    WorldGeometry? World,
    int AviWidth,
    int AviHeight,
    byte[]? AviRgba,
    int AviSerial,
    bool AviPlaying,
    byte FadeAlpha,
    byte FadeR,
    byte FadeG,
    byte FadeB,
    Fable.Render.MeshVertex[]? Vertices = null,
    Fable.Render.MeshDraw[]? Draws = null,
    Fable.Render.GpuTexture[]? Textures = null,
    Fable.Render.MeshVertex[]? ObjectVertices = null,
    Fable.Render.MeshDraw[]? ObjectDraws = null,
    ushort[]? Indices = null,
    uint AviClearArgb = 0xFF000000,
    byte[]? FrontendRgba = null,
    int FrontendWidth = 0,
    int FrontendHeight = 0,
    float PresentX0 = 0,
    float PresentY0 = 0,
    float PresentX1 = 1,
    float PresentY1 = 1,
    Fable.Render.FrontendSubmitBatch? FrontendBatch = null);

public readonly record struct FrontendWidget(
    string Name,
    int Type,
    float DestX0,
    float DestY0,
    float DestX1,
    float DestY1,
    string? TextValue,
    string? Text,
    string? ParentName = null,
    int ParentIndex = -1,
    string? TextureName = null,
    int GraphicId = 0,
    float PersistWidth = 0,
    float PersistHeight = 0,
    float PersistX = 0,
    float PersistY = 0,
    float PersistScaleX = 1,
    float PersistScaleY = 1,
    float Leftover204 = 0,
    float DestScaleX = 1,
    bool PositionIsCenter = false,
    bool Independant = false,
    bool UseRelativePosition = false,
    bool UseRelativeZoom = false,
    bool Visible = true,
    bool Enabled = true,
    byte DrawFromViewport = 0,
    int ActiveChild = 0,
    int State = 0,
    int StyleIndex = 0,
    byte Flag302 = 0,
    int Alignement = 0,
    IReadOnlyList<uint>? StyleColours = null,
    IReadOnlyList<float>? StylePositionX = null,
    IReadOnlyList<float>? StylePositionY = null,
    IReadOnlyList<float>? StyleZoomX = null,
    IReadOnlyList<float>? StyleZoomY = null,
    IReadOnlyList<float>? StyleDurations = null,
    IReadOnlyList<int>? StyleGraphicIds = null,
    IReadOnlyList<string?>? StyleTextureNames = null,
    IReadOnlyList<int>? StyleFlags = null,
    IReadOnlyList<int>? StyleChangeTypes = null,
    IReadOnlyList<bool>? StyleLinearChanges = null,
    IReadOnlyList<IReadOnlyList<int>>? StyleChildrenNotAffected = null,
    uint ColourTransitionFrom = 0,
    uint ColourTransitionTo = 0,
    float ColourTransitionElapsed = 0,
    float ColourTransitionDuration = 0,
    bool ColourTransitionActive = false,
    float TransformFromX = 0,
    float TransformFromY = 0,
    float TransformToX = 0,
    float TransformToY = 0,
    float TransformFromScaleX = 1,
    float TransformFromScaleY = 1,
    float TransformToScaleX = 1,
    float TransformToScaleY = 1,
    bool PositionTransitionActive = false,
    bool ScaleTransitionActive = false,
    IReadOnlyList<int>? SwappingStates = null,
    IReadOnlyList<float>? SwappingTimes = null,
    bool SwapTickPrimed = false,
    float SwapStateElapsed = 0,
    int SwapCurrentState = int.MinValue,
    int Font = 0,
    string? FontFace = null,
    int ActionOnLeftUnclicked = 0,
    /// <summary>
    /// Type-10 attach packet first dword
    /// at widget <c>+352</c>
    /// (<c>0054E4F0</c>). Persist
    /// <c>ActionOnLeftUnclicked</c> stays 0.
    /// </summary>
    int Type10Packet = 0,
    int ActionOnLeftClicked = 0,
    int HoveredState = 0,
    int LeftClickedState = 0,
    int RightClickedState = 0,
    float InputDelay = 0,
    bool EditBoxParentIsButton = false,
    bool PasswordBox = false,
    int EditBoxCharLimit = 0,
    bool EditBoxUsesIme = false,
    bool DisallowSpaceAsFirstChar = false,
    bool Armed = false,
    /// <summary>
    /// Native type-11/38 inner <c>+364</c>. 0055AF60 stores the state that
    /// was active when LMB went down; 0055ACF0 restores it on LMB up.
    /// </summary>
    int PressedRestoreState = 0,
    float U0 = 0,
    float V0 = 0,
    float U1 = 0,
    float V1 = 0,
    uint Colour = 0xFFFFFFFFu,
    int GlyphCount = 0,
    int DrawOrder = 0,
    int Layer = 0,
    float PositionOffsetY = 0,
    float PositionOffsetX = 0,
    int ExpansionType = 0,
    IReadOnlyList<int>? HorizontalSeparations = null,
    IReadOnlyList<int>? VerticalSeparations = null,
    float HitX0 = 0,
    float HitY0 = 0,
    float HitX1 = 0,
    float HitY1 = 0,
    float TextOriginX = 0,
    float TextOriginY = 0,
    bool Hovered = false,
    float DimensionsX = 0,
    float DimensionsY = 0,
    float MinX = 0,
    float MinY = 0,
    float MaxX = 0,
    float MaxY = 0,
    float StepX = 0,
    float StepY = 0,
    int SliderLeftDef = 0,
    int SliderRightDef = 0,
    int ControlOwnerIndex = -1,
    int ControlDirection = 0,
    int Action = 0,
    int ActionOnBack = 0,
    int ActionOnSelected = 0,
    int ActionOnUnselected = 0,
    int ActionOnDestruction = 0,
    int ActionOnLeftHeld = 0,
    int ActionOnRightClicked = 0,
    int ActionOnDropped = 0,
    int ActionOnDroppedNowhere = 0,
    int PreAction = 0,
    int ActionOnDraggedUp = 0,
    int ActionOnDraggedDown = 0,
    int ActionOnLeftClickedAbove = 0,
    int ActionOnLeftClickedUnder = 0,
    float SliderValueX = 0,
    int Sprite2DFlag = 0,
    float Angle = 0,
    bool LayerIndependant = false,
    int TableSpriteKey = -1,
    int TableRepeatIndex = 0,
    int TableRepeatCount = 1);
