...
`CompositeFrontendPresent` and the type-10 draw note walked every constructed widget with nonempty dest. Native type 10/5/12/18 `vtbl+8` is `00530260`, which walks `+176` and skips children. Type 18 (`CSwappingStateComponent`) keeps one persist child; the other blending sets stay in the tree but are not drawn.
...
## Press Start first-seen visible set (PROVEN tree + first-seen state 0)
...
| `UI_TITLE` | 5 | visible |
| `UI_PRESS_START_SWAP` | 18 | visible; child 0 only |
| `UI_FRONTEND_LIST_PRESS_START_MENU` | 12 | visible (`00530260`) |
| `UI_LEGAL_TEXT` | 6 | visible |
| `UI_MOUSE_POINTER` | 32 | visible |

Forest type-18 first persist child (visible) vs siblings (hidden):

| Swap | Active child 0 | Hidden |
| --- | --- | --- |
...
Hidden tiles stay in the widget list. Draw and present do not emit them. No string filter.

Visibility / enabled / clip inherit: a hidden type-18 sibling hides its descendants. `00530260` does not recurse a skipped child.
...
- Whether type 18 later `vtbl+192` writes `+302` bit 0 or only style keys
